...
custom_flag_checker :: proc(
	...
	if name == "script_path" {
		...
		if !os.exists(script_path) {
			return fmt.aprintf("Path '%s' was passed as 'script_path' but does not exist.", script_path)
		}
		if !os.is_file(script_path) {
			return fmt.aprintf("Path '%s' was passed as 'script_path' but is not a regular file.", script_path)
		}
	}
	return
}
...
main :: proc() {
	...
	build_user_script_command :[]string = {"odin", "build", cli_opts.script_path, "-file", "-build-mode:shared", "-out:script.so"}
	odin_build_script_proc_desc := os.Process_Desc{command = build_user_script_command}
